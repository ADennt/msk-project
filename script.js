    ...
    database.ref('products').once('value').then(snapshot => {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        fetch('data.json')
          ...
          .catch(err => {
            console.warn('Не удалось загрузить data.json, используется fallback', err);
...
function getAutoReply(text) {
  ...
  if (lower.includes('адрес') || lower.includes('где') || lower.includes('находитесь') || lower.includes('карта') || lower.includes('навигатор') || lower.includes('проехать')) {
