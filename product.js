...
function loadCart() {
    ...
    if (uid && window.database) {
        database.ref(`users/${uid}/cart`).once('value').then(snapshot => {
...
function loadProduct() {
    ...
    // Загружаем товары из localStorage (они уже синхронизированы с Firebase в script.js)
    ...
    loadCart();
    // Подписываемся на обновления корзины в реальном времени
    const uid = window.getCurrentUserId ? window.getCurrentUserId() : null;
    if (uid && window.database) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Ждём готовности Firebase, затем загружаем товар
    if (window.waitForFirebase) {
        window.waitForFirebase(loadProduct);
    } else {
        loadProduct();
    }
});