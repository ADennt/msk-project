...
function initData() {
    ...
    if (window.waitForFirebase) {
        ...
        // fallback: если Firebase не загружен, используем localStorage
...
async function loadCartFromFirebase() {
    ...
    try {
        ...
        if (data) {
            ...
            // Если в Firebase корзины нет, используем localStorage
            const saved = localStorage.getItem('msk_cart');
            cart = saved ? JSON.parse(saved) : [];
            // Сохраняем в Firebase
...
// ===== КОРЗИНА (с сохранением в Firebase) =====
...
function submitOrder() {
    ...
    // Получаем следующий ID заказа (инкрементальный) через Firebase или localStorage
    const getOrderId = async () => {
        if (window.getNextOrderId) {
            try {
                const id = await window.getNextOrderId();
                return id;
            } catch (e) {
                console.warn('Ошибка получения ID из Firebase, используем локальный', e);
                // fallback
    ...
    getOrderId().then(newId => {
        ...
        // Сохраняем заказ в localStorage и Firebase
        let orders = JSON.parse(localStorage.getItem('msk_orders') || '[]');
        orders.push(order);
